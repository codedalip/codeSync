...
const CreateFileModal = ({ isOpen, onClose, workspaceId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await createFile(workspaceId, fileName.trim());
    setIsSubmitting(false);

    if (result.success) {
      // Broadcast file created socket event
      const socket = getSocket();
      if (socket) {
        socket.emit('file:created', { workspaceId, file: result.file });
      }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="New File">
      <form onSubmit={handleSubmit} className="space-y-5 select-none">
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            placeholder="e.g. index.js, utils.py, main.cpp, App.java"
          ...
          <p className={`text-[11px] pt-1 ${isLight ? 'text-zinc-500' : 'text-zinc-400'}`}>
            Supported extensions: <span className={`font-mono font-bold ${
              isLight ? 'text-zinc-900' : 'text-white'
            }`}>.js, .py, .cpp, .c, .java, .html, .css, .json</span>
          </p>
        </div>
